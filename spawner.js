        ...
        for(let i in Game.spawns) {
            ...
            if(!spawn.spawning) {
                ...
                if(hostiles.length > 0 && (!creepsRole["guard"][hostiles[0].room.name] || creepsRole["guard"][hostiles[0].room.name].length < 1) && (dangerClass <= (spawn.room.controller.level * spawn.room.controller.level * spawn.room.controller.level * 2) || hostiles[0].room.name === spawn.room.name)) {
                    ...
                    if(Memory.datas[spawn.room.name].sources && energyDropped > (Math.max((creepsRole["hauler"][spawn.room.name].length - Object.keys(Memory.datas[spawn.room.name].sources).length), 0) * Math.max((creepsRole["hauler"][spawn.room.name].length - Object.keys(Memory.datas[spawn.room.name].sources).length), 0)) * Math.max(150 + spawn.room.controller.level * spawn.room.controller.level * 10, 550) + 350) {
                        let creepName = 'RH' + (Game.time % 1651) + spawn.room.name;
